...
int main()
{
    ...
    //Search(head); 				         成功
    //PrintVoter(head); 				     成功
    //PrintVote(head1, head2);  		     成功
    //AddVoter(head, head1, head2);		     成功
    //DeleteVoter(head, head1, head2); 	     成功
    //ReviseInformation(head, head1, head2); 成功
    PrintTop(head1, head2);
    //Save(head, head1, head2);
}
...
A:
        cout << "请输入想修改的选项和修改后的选项 (中间空格分隔)：" << endl;
        cin >> pre >> now;
...
B:
    ...
    if (hao == "a")
    {
        ...
        while (i!=j)
        {
            if (pre -> votes < pre->next-> votes)
            {
                head1->next=head1->next->next;
                j->next=head1;
                head1=j;
            }
            for (pre = head1,j=pre->next; j -> next != i; j = j -> next  )
            {
                if (j -> votes < j->next-> votes)
                {
                    pre->next=j->next;
                    j->next=j->next->next;
                    pre->next->next=j;
